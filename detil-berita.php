          ...
          <div class="col-md-9">
			...
			<?php
				if(isset($_GET['berita'])){
					$judul_wakaf = str_replace("-"," ",$_GET['berita']);
					$stmt = $mysqli->query("select * from tbl_post where status_post='done' AND tipe_post='berita' AND judul_post='".$judul_wakaf."'");
					if($stmt->num_rows>0){
						...
						echo'
						  ...
						  <article class="post clearfix mb-0">
							<div class="entry-header">
							  <div class="post-thumb thumb"> <img src="'.$data->url_foto.'" alt="" class="img-responsive img-fullwidth"> </div>
							</div>
                  ...
                  <div class="col-sm-12">
					...
					<?php
						if(isset($_POST['btnPostKomentar'])){
							...
							if($stmt){
								echo'<meta http-equiv="Refresh" content="0; URL=?wakaf='.$_GET['berita'].'">';
								echo '		
            ...
            <div class="sidebar sidebar-left mt-sm-30">
              ...
              <div class="widget">
                ...
                <div class="latest-posts">
                  <?php
						$stmt = $mysqli->query("select * from tbl_post where status_post='done' AND (tipe_post='tausyiah' OR tipe_post='berita') order by id_post DESC LIMIT 0,5");
						while($data = $stmt->fetch_object()){
							echo'
							  <article class="post media-post clearfix pb-0 mb-10">
								<a class="post-thumb" href="#"><img src="'.$data->url_foto.'" width="75" height="75" alt=""></a>
								<div class="post-right">
								  <h5 class="post-title mt-0"><a href="detil-'.strtolower($data->tipe_post).'.php?'.strtolower($data->tipe_post).'='.str_replace(" ","-",$data->judul_post).'">'.$data->judul_post.'</a></h5>
								  <p>'.substr($data->isi_post,0,30).'...</p>
								</div>
							  </article>
							';
						}
