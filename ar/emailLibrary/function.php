...
function smtp_mail($to, $subject, $message, $from_name, $from, $cc, $bcc, $debug=false) {
  ...
  $mail->SMTPAuth 	= true;                        				
  $mail->Host 		  = 'woi.or.id';  // Masukkan Server SMTP
  $mail->Port 		  = 465;                                      // Masukkan Port SMTP
  $mail->SMTPSecure = 'ssl';                                    // Masukkan Pilihan Enkripsi ( `tls` atau `ssl` )
  $mail->Username 	= 'no-reply@woi.or.id';                // Masukkan Email yang digunakan selama proses pengiriman email via SMTP
  $mail->Password 	= 'V=~(&f;DaT$w';        						          // Masukkan Password dari Email tsb
  $default_email_from       = 'no-reply@woi.or.id';        // Masukkan default from pada email
  $default_email_from_name  = 'Wakaf Orang Indonesia';           // Masukkan default nama dari from pada email
